...
const sslSetting = dbConfig.ssl
const dbUrl = dbConfig.connectionString
...
// return entries with author id or name and identity???
  const client = new Client({ connectionString: dbUrl, ssl: sslSetting })
  client.connect()
...
// done: update lastUpdated whenever u update an entry
...
router.post('/', async (req, res) => {
  ...
  const { term, definition, timeSubmitted, action } = req.body
  ...
  try {
    ...
    // //insert entry!
    await client.query(entryQueryString, [term, definition, explanation, authorId, action, timeSubmitted, lastUpdated])
...
router.post('/setstatus/:action/id/:id', async (req, res) => {
  ...
  var lastUpdated = new Date().toISOString()
  var entryQueryString = 'UPDATE entry SET action=$1, lastUpdated=$2 WHERE entry_id=$3'

  try {
    await client.query(entryQueryString, [action, lastUpdated, id])
