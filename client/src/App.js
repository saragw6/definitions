...
class App extends Component {
  ...
  handleTermChange = (value) => {
    var newQuery = value === '' ? '' : 'search/' + encodeURIComponent(value)
    this.setState({ my_term: value })
    // history.pushState(null, null, "/" + newQuery); //add term to url
      ...
      <DocumentTitle title={pageTitle}>
        <ThemeProvider theme={theme}>
          <div>
            ...
            {(!showLoading && this.state.my_term !== '' && myEntries.length === 0) &&
            ...
            <ResultList style={{ display: 'flex', flexDirection: 'column', alignContent: 'center' }} entries={myEntries} />
